...
Pokémon in the games Pokémon X and Y are stored in a new 232 byte structure (for the box data only).
Party Pokémon hold 28 bytes of additional data for overworld and battle values.
We recommend using the pkx extension for decrypted 232-byte box data and 260-byte party data structures.

Contents [hide]
1 Checksum / Block Shuffling / Encryption
2 Unencrypted bytes (0-8)
3 Encrypted bytes
3.1 Block A (8-40h)
3.2 Block B (40-78h)
3.3 Block C (78-B0h)
...
There have been some changes compared to the Generation 4 and Generation 5 structures.
First, the LCRNG to decrypt the structure's blocks is now seeded with the PID.
Second, the blocks are now 56 bytes instead of the previous 32.
However, the way the blocks are shuffled based on the PID did not change.
